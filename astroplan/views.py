...
#
#
#     form = ChartForm(request.POST or None, request.FILES or None)
#     if form.is_valid():
#
#         # date = dt.strptime(str(Chart.chart_date), '%Y-%m-%d %H:%M:%S')
#         # jl.to_jd(Chart.chart_date,fmt='jd')
#         # getLoc = loc.geocode(f'{str(Chart.country,Chart.city)}', timeout=7000)
#         # houses = swe.houses_ex(jd, getLoc.latitude, getLoc.longitude, b'R', flags=swe.FLG_SIDEREAL)
#         form.save()
#
...
#                             'att': aspect_table_t, 'atc': aspect_table_c, 'form':form})
