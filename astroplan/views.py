...
from .forms import ChartForm, ShowChart, TransitForm, ColorfulZodiacForm, OneColorZodiacRing
...
def show_td_chart(request):
    ...
    return render(request, 'main_astro.html',
                  context={'chart_form': chart_form, 'planet_data': set_signs(planet_names, [p[4] for p in coords_value]),
...
def chart_for_any_date(request):
    ...
    if chart_form.is_valid():
         ...
         if house_system != 'Without houses':
             ...
             return render(request, 'show_by_date.html', {'planet_data': set_signs(planet_names,
                                                                                         [p[4] for p in
                                                                                          form_coords_value]),
                                                                'house_data': set_signs(house_names, list(houses[0])),
                                                                'ats': aspect_table_s, 'ato': aspect_table_ops,
...
def build_transit_chart(request):
    ...
    if tr_form.is_valid():
        ...
        tr_mode = tr_form.cleaned_data['ev_mode']
        tr_hs = tr_form.cleaned_data['ev_house_system']
        ...
        pd_cr = {swe.get_planet_name(0): ['☼', 'yellow', 9, 30, swe.calc_ut(jd_ev, 0, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 0, int(tr_mode))[0][1], 10],
                 swe.get_planet_name(1): ['☾', 'blue', 9, 30, swe.calc_ut(jd_ev, 1, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 1, int(tr_mode))[0][1], -25],
                 swe.get_planet_name(2): ['☿', 'grey', 9, 30, swe.calc_ut(jd_ev, 2, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 1, int(tr_mode))[0][1], -25],
                 swe.get_planet_name(3): ['♀', 'sienna', 9, 30, swe.calc_ut(jd_ev, 3, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 1, int(tr_mode))[0][1], 25],
                 swe.get_planet_name(4): ['♂', 'red', 9, 30, swe.calc_ut(jd_ev, 4, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 1, int(tr_mode))[0][1], -10],
                 swe.get_planet_name(5): ['♃', 'teal', 9, 30, swe.calc_ut(jd_ev, 5, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 1, int(tr_mode))[0][1], 0],
                 swe.get_planet_name(6): ['♄', 'slategrey', 9, 30, swe.calc_ut(jd_ev, 6, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 1, int(tr_mode))[0][1], -25],
                 swe.get_planet_name(7): ['♅', 'chartreuse', 9, 30, swe.calc_ut(jd_ev, 7, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 1, int(tr_mode))[0][1], 0],
                 swe.get_planet_name(8): ['♆', 'indigo', 9, 30, swe.calc_ut(jd_ev, 8, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 1, int(tr_mode))[0][1], 0],
                 swe.get_planet_name(9): ['⯓', 'darkmagenta', 9, 30, swe.calc_ut(jd_ev, 9, int(tr_mode))[0][0],
                                          swe.calc_ut(jd_ev, 1, int(tr_mode))[0][1], 0]}

        form_coords_value = list(pd_cr.values())

        pd = {swe.get_planet_name(0): ['☼', 'yellow', 9, 30, swe.calc_ut(jd_tr, 0, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 0, int(tr_mode))[0][1], 10],
              swe.get_planet_name(1): ['☾', 'blue', 9, 30, swe.calc_ut(jd_tr, 1, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 1, int(tr_mode))[0][1], -25],
              swe.get_planet_name(2): ['☿', 'grey', 9, 30, swe.calc_ut(jd_tr, 2, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 1, int(tr_mode))[0][1], -25],
              swe.get_planet_name(3): ['♀', 'sienna', 9, 30, swe.calc_ut(jd_tr, 3, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 1, int(tr_mode))[0][1], 25],
              swe.get_planet_name(4): ['♂', 'red', 9, 30, swe.calc_ut(jd_tr, 4, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 1, int(tr_mode))[0][1], -10],
              swe.get_planet_name(5): ['♃', 'teal', 9, 30, swe.calc_ut(jd_tr, 5, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 1, int(tr_mode))[0][1], 0],
              swe.get_planet_name(6): ['♄', 'slategrey', 9, 30, swe.calc_ut(jd_tr, 6, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 1, int(tr_mode))[0][1], -25],
              swe.get_planet_name(7): ['♅', 'chartreuse', 9, 30, swe.calc_ut(jd_tr, 7, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 1, int(tr_mode))[0][1], 0],
              swe.get_planet_name(8): ['♆', 'indigo', 9, 30, swe.calc_ut(jd_tr, 8, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 1, int(tr_mode))[0][1], 0],
              swe.get_planet_name(9): ['⯓', 'darkmagenta', 9, 30, swe.calc_ut(jd_tr, 9, int(tr_mode))[0][0],
                                       swe.calc_ut(jd_tr, 1, int(tr_mode))[0][1], 0]}
        ...
        if ev_hs and tr_hs != 'Without houses':
            ...
            houses = swe.houses_ex(jd_ev, get_loc.latitude, get_loc.longitude, ev_hs_bytes, int(tr_mode))
            house_ax.set_thetagrids(houses[0],
                                    ['ASC', 'II', 'III', 'IC', 'V', 'VI', 'DSC', 'VIII', 'IX', 'MC', 'XI', 'XII'])
            house_ax.tick_params(labelsize=20, grid_color='aliceblue', grid_linewidth=1,
...
def chart_form(request):
    ...
    if chart_form.is_valid():
        ...
        pd = {swe.get_planet_name(0): ['☼', 'yellow', 9,30, swe.calc_ut(jd, 0, flags)[0][0],
              ...
              swe.get_planet_name(3): ['♀', 'sienna', 9,30, swe.calc_ut(jd, 3, flags)[0][0],
                                       swe.calc_ut(jd, 1, flags)[0][1]],
              swe.get_planet_name(4): ['♂', 'red', 9,30, swe.calc_ut(jd, 4, flags)[0][0],
                                       swe.calc_ut(jd, 1, flags)[0][1]],
              swe.get_planet_name(5): ['♃', 'teal', 9,30, swe.calc_ut(jd, 5, flags)[0][0],
                                       swe.calc_ut(jd, 1, flags)[0][1]],
              swe.get_planet_name(6): ['♄', 'slategrey', 9,30, swe.calc_ut(jd, 6, flags)[0][0],
              ...
              swe.get_planet_name(8): ['♆', 'indigo', 9,30, swe.calc_ut(jd, 8, flags)[0][0],
                                       swe.calc_ut(jd, 1, flags)[0][1]],
              swe.get_planet_name(9): ['♇', 'darkmagenta', 9,30, swe.calc_ut(jd, 9, flags)[0][0],
                                       swe.calc_ut(jd, 1, flags)[0][1]]
              }
        coords_value = list(pd.values())
...
def one_color_chart(request):
    ...
    if zodiac_oc_form.is_valid():
        ...
        if zf_hs != 'Without houses':
            ...
            houses = swe.houses_ex(jd, get_loc.latitude, get_loc.longitude, hs_en, flags=swe.FLG_SIDEREAL)
